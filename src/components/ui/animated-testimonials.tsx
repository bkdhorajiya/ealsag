    ...
    >
      <div className="relative grid grid-cols-1 md:grid-cols-2 gap-20">
              ...
              {testimonials.map((testimonial, index) => (
                ...
                >
                  <Image
                    ...
                    height={500}
                    draggable={false}
                    className={`h-full w-full ${testimonial.src === '/iitbh.png' ? 'rounded-full' : 'rounded-3xl'} object-cover object-center`}
                  />
                </MotionDiv>
              ))}
            </AnimatePresence>
        ...
        <div className="flex justify-between flex-col py-4">
          ...
          >
            <h3 className="text-2xl font-bold text-blue-300 dark:text-blue-700">
              {testimonials[active].name}
            </h3>
            <p className="text-sm text-gray-100 dark:text-neutral-100">
              {testimonials[active].designation}
            </p>
            <MotionP className="text-lg text-gray-100 mt-8 dark:text-neutral-100">
...
const MotionP: React.FC<MotionDivProps> = ({ 
  className, 
  ...props 
}) => {
  return (
